...
class Emails_model extends CI_Model
{
     ...
     * @return    array
     */
    public function get_ready_email_from_template($template_id = '', $case_id = '', $country_id = '', $files = array(), $status_id = -1, $email_type = '', $fa_id = "", $zip_hash = '')
    {
        $ci = & get_instance();
        ...
        if (!is_null($template = $this->get_template($template_id))) {
            ...
            if (!is_null($case = $ci->cases->get_case($case_id, FALSE, TRUE))) {
                ...
                // FA data
                    $fa = $ci->cases->get_associate_by_id($fa_id);
                if($fa){
                    $fa_currency_sign = '$';
                    ...
                    //var_dump($countries_list);exit;
                   // $fa_country = $fa['country'];
                    $fa_country = $countries_list;
