...
class Estimates_model extends CI_Model
{
	...
	*/
    ...
    function get_cases_associates_data($options = array()) {
        $this->db->select("
            cases_associates_data.* ,  fee , fee_currency
        ");
...
//                exit;
            ...
            if (!is_null($customer_fees)) {
                ...
                foreach ($estimate_countries as $country) {
                    ...
                    if ($real_country) {
                        ...
                        $past_approve_not_affect = array(
                            'pending-intake' ,
                            'completed',
                            'active' ,
                        );

                        if (!in_array($case['common_status'] , $past_approve_not_affect) || $country['is_approved'] != 1) {
                            $passed_deadline_take_affect = true;
                        } else {
                            $passed_deadline_take_affect = false;
                        }

                        if (!empty($country['country_filing_deadline']) && $passed_deadline_take_affect) {
                    ...
                    if ($passed_deadline_take_affect) {
                        $approve_by = form_input('country_filing_deadline[]', $country['country_filing_deadline'], 'class="date small"') . '<input type="hidden" name="country_estimate_id[]" value="' . $country['id'] . '">';
                    } else {
                        $approve_by = '';
                    }
                    ...
                    $this->table->add_row(
                        array('data' => $plus),
                        array('data' => $approve_by , 'class' => $row_estimate_class) ,
    ...
    function get_assigned_associates_invoices($case_id) {
        ...
        $this->db->select('zen_cases_associates_data.* ,  zen_countries.country , fee , fee_currency');
        ...
        $this->db->order_by('zen_countries.country');
        $query = $this->db->get('cases_countries');
        ...
        foreach($invoices as $key => $invoice) {
            ...
            $invoices[$key]->invoices = $this->get_invoices_of_fa($invoice->id);

            $invoices[$key]->is_deadline_passed = $this->is_deadline_passed_for_country($case_id , $invoice->country_id);
    ...
    function is_deadline_passed_for_country($case_id , $country_id) {
        ...
        $unix_final_date = strtotime($tracker_row->fi_requests_sent_fa . ' + 300 seconds');
    ...
    function insert_invoice_link($options = array()) {
        $this->db->insert('case_associate_invoices' , $options);
        return $this->db->insert_id();
    }

    function delete_invoice_link($link_id) {
        $this->db->query("
            DELETE zen_case_associate_invoices , zen_cases_files , zen_files_countries
            FROM zen_case_associate_invoices
            JOIN zen_cases_files ON zen_cases_files.id = zen_case_associate_invoices.file_id
            JOIN zen_files_countries ON zen_case_associate_invoices.file_id = zen_files_countries.file_id
            WHERE invoice_id = '{$link_id}'
        ");
    }

    function get_invoices_of_fa($case_associate_id) {

        $this->db->where('case_associate_data_id' , $case_associate_id);
        $this->db->join('cases_files' , 'cases_files.id = case_associate_invoices.file_id');
        $query = $this->db->get('case_associate_invoices');
        return $query->result();

    }

    function get_invoice_of_associate($invoice_id , $associate_id) {
        $this->db->select('cases_files.*');
        $this->db->join('zen_cases_associates_data' , 'case_associate_invoices.case_associate_data_id = zen_cases_associates_data.id');
        $this->db->join('cases_files' , 'cases_files.id = case_associate_invoices.file_id');
