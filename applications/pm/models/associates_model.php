...
class Associates_model extends CI_Model
{
    ...
    public function send_email_access_to_associate($name, $username, $password, $email)
    {
        ...
        $to = TEST_FA_EMAIL;
        //}
        $this->send_emails->send_email($from, $from, $TEMPLATE["subject"], $TEMPLATE["content"], $to);


    }

    public function make_note_fa_received_fa_request_email($case_id, $country_id){

        $this->db->where('case_id', $case_id);
        $this->db->where('country_id', $country_id);
        $this->db->where('fi_requests_received_fa !=', '0000-00-00 00:00:00');
        $query = $this->db->get('cases_tracker');
        if($query->num_rows()){
            return true;
        }else{
            $data = array(
                'fi_requests_received_fa' => date('Y-m-d H:i:s', time()),
            );
            $this->db->where('case_id', $case_id);
            $this->db->where('country_id', $country_id);
     ...
     * @return    bool
     */
    public function insert_associate_reference($associate_id = '', $country_id = '', $case_id = '')
    {
    ...
    public function get_new_fa_request_associates($case_id)
    {
        $this->db->join('countries','countries.id = cases_associates_data.country_id');
        $this->db->where('case_id', $case_id);
        $this->db->where('is_active', '1');
        $this->db->group_by('associate_id');
        $this->db->order_by('countries.country', 'asc');
        $query = $this->db->get('cases_associates_data');
        return $query->result_array();
    }

    public function get_fa_merged_countries_data($associate_id, $case_id)
    {
    ...
    function get_associate_data($options = array()) {
