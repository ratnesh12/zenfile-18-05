...
class Cases_model extends CI_Model
{
     ...
     * @return    bool
     * */
    public function delete_custom_associate($custom_associate_id, $case_id, $country_id)
    {
        $this->db->where('associate_id', $custom_associate_id);
        $this->db->where('case_id', $case_id);
        $this->db->where('country_id', $country_id);
    ...
    public function get_approved_regions_for_all_cases()
    {
        ...
        $q = 'SELECT c.id, cs.code
			  ...
			  WHERE (c.id = ecf.case_id) AND
			  		(cs.id = ecf.country_id) AND parent_id = 0';
        $query = $this->db->query($q);
        if ($query->num_rows()) {
    ...
    public function get_list_files_by_email_type()
    {
        ...
        if (!is_null($case = $this->find_case_by_number($case_number))) {
            ...
            $new_fa = $this->associates->get_new_fa_request_associates($case['id']);
            ...
            if (!empty($new_fa)) {
                ...
                //											</div>';
                //                }
                foreach ($new_fa as $record) {
                    $country_data = $this->associates->get_fa_merged_countries_data($record['associate_id'], $case['id']);
                    //var_dump($country_data);
                    $tmp_country = array();
                    foreach ($country_data as $tmp) {
                        $tmp_country[] = $tmp['country'];
                    }
                    $countries_list = implode('/', $tmp_country);
                    // var_dump($countries_list);
                    $country_class = (!empty($sent_email_result_final[$record['country_id']]['created_at'])) ? 'sent' : '';
                    $result['countries'] .= '<div class="country-box ' . $country_class . '">
                    												<a id="' . $record['country_id'] . '" rel="' . $record["associate_id"] . '" class="email-country" href="javascript:void(0);">' . $countries_list . '</a>' .
                        //                        form_hidden('translation_needed_' .$record['country_id'], $translation_required) .
                        form_hidden('country_association_id_' . $record['country_id'], $record["associate_id"]) . '
                    											</div>';
                }
    ...
    public function get_email_text()
    {
        ...
        $fa_id = $this->input->post("country_association_id");
        $cc = $this->input->post('cc');
        $case_id = $this->input->post('case_id');
        $case_number = $this->input->post('case_number');
        $case_type_id = $this->input->post('case_type_id');
        $country_id = $this->input->post('country_id');
        //var_dump($country_id);exit;
        $email_type = $this->input->post('email_type');
        $files = $this->input->post('files');

        if (!empty($fa_id)) {

            $this->db->join('cases_countries', 'cases_countries.case_id = cases_associates_data.case_id and zen_cases_countries.country_id = zen_cases_associates_data.country_id');
            $this->db->where('cases_countries.extension_needed', '1');
            ...
            $this->db->join('cases_tracker', 'cases_tracker.case_id = cases_associates_data.case_id and zen_cases_tracker.country_id = zen_cases_associates_data.country_id');
        ...
        $files = NULL;
        // FA data
        $fa = $this->get_associate_by_id($fa_id);
        ...
        if (!empty($fa_id)) {
            ...
            $this->db->join('cases_associates_data', 'cases_tracker.case_id = cases_associates_data.case_id and zen_cases_tracker.country_id = zen_cases_associates_data.country_id');
            $this->db->where('cases_associates_data.case_id', $case_id);
            $this->db->where('cases_associates_data.associate_id', $fa_id);
            $this->db->where_in('cases_files.file_type_id', $file_types);
        ...
        $text = $ci->emails->get_ready_email_from_template($template_id, $case_id, $country_id, $files, 777, $email_type, $fa_id, $zip_hash);
    ...
    public function send_email()
    {
        ...
        if (!is_null($customer = $this->get_customer_by_case_number($case_number))) {
            if (!is_null($case = $this->find_case_by_number($case_number))) {
                    ...
                    if ($email_type == 'intake-email' || $email_type == 'fa-request') {
                        ...
                        if (@mkdir($case_dir . '/')) {
                            ...
                            if ($email_type == 'fa-request') {
                                ...
                                $country_data = $this->associates->get_fa_merged_countries_data($fa_id, $case['id']);
                                //var_dump($country_data);
                                $tmp_country = array();
                                foreach ($country_data as $tmp) {
                                    $tmp_country[] = $tmp['id'];
                                    $this->save_tracker($case['id'], $tmp['id'], 'fi_requests_sent_fa', 'current_date');
                                }
                                $country_id = implode(',', $tmp_country);
                ...
                if ($ci->send_emails->send_email($from, $from, $subject, $text, $send_to, $send_cc, $attachments, true)) {
    ...
    public function send_notification_email()
    {
        ...
        // $zip_hash = $this->input->post('zip_hash');
        $country_id = $this->input->post('country_id');
        $case_files = $this->input->post('files');
        ...
        if (!is_null($customer = $this->get_customer_by_case_number($case_number))) {
            if (!is_null($case = $this->find_case_by_number($case_number))) {
                ...
                if (!is_null($country = $ci->countries->get_country($country_id))) {
                    ...
                    if (!empty($case_files)){
                        $files = $this->files->get_file_by_id($case_files);
                        foreach ($files as $file) {
                ...
                $text = str_replace('%CASE_FILES%', $case_files, $text);
                if ($ci->send_emails->send_email($from, $from, $subject, $text, $to, $send_cc, false, true)) {
                    // Insert data about sent email to database
                    $manager_id = $this->session->userdata('manager_user_id');
                    $data = array(
                        'case_id' => $case['id'],
...
';
        ...
        if (!empty($USER) && !empty($CASE)) {
            if ($USER['allow_email'] == 'yes') {
                ...
                if (!empty($TEMPLATE)) {
                    ...
                    if ($this->send_emails->send_email($from, $from, $TEMPLATE["subject"], $TEMPLATE["content"], $to, $cc)) {
                        return true;
                    }
                }
            }
    ...
    {
        $this->db->select('tracker_additional_data.doc_pending,tracker_additional_data.examenation ,tracker_additional_data.application_number, cases_tracker.*');
        $this->db->join('tracker_additional_data','tracker_additional_data.id = cases_tracker.tracker_additional_data_id','left');
