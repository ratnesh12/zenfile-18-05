...
class Associates extends CI_Controller
{
    ...
    public function edit($associate_id = '')
    {
        ...
        if (empty($associate_id)) {
            ...
            $this->form_validation->set_rules('username', 'Username', 'required|callback__username_check|alpha_dash');
        }
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if (empty($associate_id) || !empty($_POST['reset_password'])) {
        ...
        $associate = $this->associates->get_associate($associate_id);
        ...
        if (!empty($_POST)) {
            ...
            $data['associate']['is_replaced'] = $associate['is_replaced'];
        }
        if (!empty($associate_id)) {
    ...
    public function search_replaced_associates($case_number, $country_id, $associate_id)
    {
        ...
        if (($associates)) {
            foreach ($associates as $associate) {
                if ($associate['name']) {
                    $associates_array[] = array('id' => $associate['id'], 'firm' => $associate['firm'].' ('.$associate['country'].')');
                }
            }
        }
    ...
    public function login($associate_id){
        $this->load->model('associates_model', 'associates');
        if (!is_null($data = $this->associates->get_associate($associate_id))){
            $session_array = array(
                'fa_user_id' => $data['id'],
                'fa_name' => $data['name'],
            ...
            echo '<script type="text/javascript">
					window.location.href = "https://' . $_SERVER["HTTP_HOST"] . '/client/fa/";
				  </script>';
        }else{
            redirect('/dashboard');
        }
