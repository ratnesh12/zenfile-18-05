...
class Cases extends CI_Controller
{
    ...
    public function fc_send_email($case_id = "", $country_id = "")
    {
        ...
        $country_files = $this->cases->get_case_files_by_country($case['id'], $country_id, array(6));
        //var_dump($country_files);exit;
        $data = array(
            'customer' => $customer,
            'country' => $country,
    ...
    public function submit_intake($case_number = '')
    {
        ...
        if (!is_null($case = $this->cases->find_case_by_number($case_number))) {
            ...
            $this->send_emails->send_email($from, $from, $subject, $email_content, $send_to, $send_cc, $attachments, true);
            redirect('/cases/view/' . $case_number);
        }
    }
    ...
    {
        $this->load->model('cases_model', 'cases');
        $this->cases->delete_custom_associate($custom_associate_id, $case_id,$country_id);
        $case_number = $this->cases->get_case_number($case_id);
        redirect('/cases/view/' . $case_number['case_number']);
    }
     ...
     * @return    void
     */
    public function associate_reference_form($case_id, $country_id, $associate_id)
    {
        $this->load->model('associates_model', 'associates');
        $data['case_id'] = $case_id;
        $data['country_id'] = $country_id;
     ...
     * @return    void
     */
    public function associate_reference_form_submit($case_id, $country_id, $associate_id)
    {
        $this->load->model('associates_model', 'associates');
        $this->load->model('cases_model', 'cases');
        $this->associates->insert_associate_reference($associate_id,$country_id, $case_id, TRUE);
        $case_number = $this->cases->get_case_number($case_id);
        redirect('/cases/view/' . $case_number['case_number']);
    }
    ...
    {
        $this->load->model('files_model', 'files');
        $data['file_data'] = $this->files->file_view_more($file_id);
        //var_dump($data);
        $this->load->view('/cases/file_view_more',$data);
    }
    ...
    function _edit_invoice() {
        ...
        switch ($this->input->post('fa_invoice_status')) {
            ...
            case 'pending-unlock':
                $options['fa_invoice_status'] = 'unlocked';
                break;
            default:
                break;
        }
    ...
    function download_file() {
        ...
        $file = $this->files_model->get_files(array(
            'id' => $file_id
        ));
    ...
    public function case_files_archive(){
        $this->load->model('files_model','files');
        $this->load->library('zip');
        $this->load->helper('download');
        $case_files = $this->input->post('files_ids');
        $case_number = $this->input->post('case_number');
        $path = 'uploads/tmp/case_files' .$case_number . '.zip';
        $result = 'failed';
        if (!empty($case_files)){
    ...
    public function case_files_archive_download($case_number){
       // var_dump($case_number);exit;
        $this->load->helper('download');
        $path = 'uploads/tmp/case_files' .$case_number . '.zip';
