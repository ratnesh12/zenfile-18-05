...
class Admin extends CI_Controller
{
    ...
    public function ajax_get_usersdata(){
        $user_id = $this->input->post('user_id');
        $this->load->model('customers_model','customers');
        $customer = $this->customers->get_user($user_id, $type = 'customer');
        $result['userdata'] = 'There is no such user';
        if(!empty($customer)){
            $result['userdata'] = $customer['username'].', '.$customer['firstname'].' '.$customer['lastname'];
        }
