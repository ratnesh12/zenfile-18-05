...
<body>
	<script type="text/javascript">
		$(document).ready(function () {
			...
			$("#fc_email_send_button").click(function () {
		        ...
		        $.post("<?php echo base_url(); ?>cases/send_notification_email/", {cc:cc, case_number:case_number, text:text, to:to, subject:subject, country_id:country_id, zip_hash:zip_hash,files:files}, function (result) {
		            showNotification({
		                message:result.text,
		                type:result.type,
		                autoClose:true,
	...
	<div id="notification_email_filing_confirmation">
        ...
        if($country_files){?>
           ...
           <?php foreach($country_files as $file){?>

                <span id="attached_<?=$file['file_id'];?>" class="attachtocountry">
                <input type="hidden" value="<?=$file['file_id'];?>" name="attached_files[]">
                <a onclick="remove_file_from_email(<?=$file['file_id'];?>)" href="javascript:void(0);">
                <img title="Remove file" src="<?php echo base_url(); ?>assets/images/i/delete.png">
                </a>
                <a href="<?php echo base_url(); ?>cases/view_file/<?=$file['file_id'];?>" title="View file"><?=$file['filename'];?></a>
                <br>
                </span>
