	...
	function get_files_table(block_name){
		$.post(
	        ...
	        function(html){
                ...
                $('a#download_archive').click( function () {
					var files_ids = [];
					if($('#case-files input[name="case_files[]"]:checked').length<1){
						alert('Please, choose the files');
						return false;
                    ...
                    $.post("<?php echo base_url(); ?>cases/case_files_archive/", { files_ids: files_ids,case_number:'<?= $case['case_number'];?>'},
                    function(result){
                        if(result == 'success'){
                            window.location = "<?php echo base_url(); ?>cases/case_files_archive_download/<?= $case['case_number'] ?>";
                        }
                    },'json');
...
<? } ?>
    ...
    $("a#send-email").click(function () {
        ...
        var fa_id = $("div#email-countries div.selected a").attr("rel");
        var to = $("input#to").val();
        var cc = $("input#cc").val();
            ...
            <script id="templateUploaderEmail" type="text/x-jquery-tmpl">
                ...
                <script id="templateUploadedFile_Info" type="text/x-jquery-tmpl">
                    ...
                    $().ready(function(){
                        ...
                        $('.translation_expected_delivery_date, .fr_filing_date, .examenation').datepicker({
                            "dateFormat":"mm/dd/y",
                            "changeMonth":true,
                            "changeYear":true
                        });
                    ...
                    <div id="case-tracker">
                        <div id="tracker-tabs">
                            <ul>
                                ...
                                <li><a href="#tracker-post-filing-data"> Post Filing Data</a></li>
                            </ul>
...
?>
                    ...
                    <div id="tracker-post-filing-data">
                        <div id="tracker-table">
                        <table>
                                        <tr>
                                            <th>Countries</th>
                        ...
                        if ($countries)
                        {
                            foreach ($countries as $country):
                                ...
                                if (!in_array($country['id'], $unapproved_countries))
                                {
                                    ...
                                    if($tracker[$country['id']]['doc_pending']){

                                        if($tracker[$country['id']]['doc_pending'] == '1'){
                                           $doc_pending =  'tracker_required';
                                        }else{
                                            $doc_pending = 'tracker_not_required';
                                        }
                                    }else{
                                        $doc_pending =  'tracker_inactive';
                                    }

                                    $time = strtotime($tracker[$country['id']]['examenation']);
                                    $examenation = ($time > 0) ? 'tracker_required' : 'tracker_inactive';
                                    $examenation_date = ($time > 0) ? date('m/d/y', $time) : '';
                                    ?>
                                <tr>
                                                        <td><?= $country['country'] ?></td>

                                                        <td><button class="<?= $doc_pending ?> tracker_ajax" id="<?= $country['id'] ?>" name="doc_pending"></button></td>
                                                        <td><input type="text" class="examenation" value="<?= $examenation_date ?>" name="examenation" rel="<?= $country['id'] ?>" /></td>
                                                        <td><input type="text" class="post_fil_app_number" value="<?= isset($tracker[$country['id']]['application_number'])? $tracker[$country['id']]['application_number'] : ''; ?>" name="post_fil_app_number" rel="<?= $country['id'] ?>" /></td>
                                                    </tr>
                                    <?php
                                }
                                ?>
...
//                                    $associate_class = 'custom-fa-only';
                                    ...
                                    $check_box = '<input id="associates_for_merge" type="checkbox" name="associates[]" value="'. $associate['associate_id'] .'">';
                                        ...
                                        $link = array(
                                            'data' => anchor('/cases/edit_replaced_associate/' . $associate['associate_id'] . '/' . $case['case_number'] . '/' . $associate['country_id'] . "/", 'Edit', 'class="popup"') . '&nbsp;' . anchor('/cases/delete_custom_associate/' . $associate['associate_id'] . '/' . $case['id']. '/' . $associate['country_id'], 'Remove'),
                                        ...
                                        if($associate['is_active'] == '0'){
                                           ...
                                           //$check_box = '';
                                            ...
                                            if($associate['is_merged'] =='1'){
                                                $check_box = '';
                                                if($this->session->userdata('type') == 'supervisor'){
                                                    $final_link =  anchor('/cases/delete_custom_associate/' . $associate['associate_id'] . '/' . $case['id']. '/' .$associate['country_id'], 'Unmerge', 'class=" REPLACEMENT"');
                                                }else{
                                                    $final_link = '';
                                                }
                                                ...
                                                $final_link = $replace_link . $ENABLE_DISABLE_BUTTON;
                                            }
                                        $link = array(
                                            'data' => $final_link,
...
//                                                        'data' => $fa_data,
                                                        ...
                                                        'data' => anchor('/cases/associate_reference_form/' . $case['id'] . '/' . $associate['country_id'] . '/' . $associate['associate_id'], $reference_number_link_text, 'class="popup"'),
                                                        'class' => $associate_class
                                                            ), $link
                                                    );
