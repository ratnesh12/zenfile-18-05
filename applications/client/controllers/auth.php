        ...
        if ($type = $this->app_security_model->do_login()) {
            ...
            if ($type == 'supervisor' || $type == 'pm') {
                ...
                if (!empty($previous_url)) {
                    ...
                    redirect('https://' . $this->input->server('HTTP_HOST') . '/pm/' . $previous_url);
                }
                else
                {
                    redirect('https://' . $this->input->server('HTTP_HOST') . '/pm/dashboard/');
                }
            }
            elseif ($type == 'admin')
            {
                redirect('https://' . $this->input->server('HTTP_HOST') . '/pm/admin/');
            }elseif($type == 'fa'){
                redirect('/fa/');
            }
            else
