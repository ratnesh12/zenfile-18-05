...
<script type="text/javascript">
    ...
    function refresh_invoices() {
        $.ajax({
            type: "POST",
            cache: false,
            url: site_url + "/fa/get_invoices_table/<?php echo $associate_data->id ?>" ,
            data: {
    ...
    $(document).ready(function () {
        ...
        $('.delete_invoice').live('click' , function(){
            ...
            $.ajax({
                type: "POST",
                cache: false,
                url: site_url + "fa/remove_invoice" ,
                data: {
       ...
       $('#filing-conf_link').click(function(){
        <?php if (!empty($filing_files)){?>
        ...
        function show(){
            ...
            <?php if (!empty($document_requirements_files)){?>
        ...
        $('.confirm_filing_confirmation').click(function(){
            ...
            $.post("<?php echo base_url(); ?>fa/filing_confirmation/", {country_id: id, case_id: case_id, fa_id: fa_id, examenation:examenation,fc_filing_date:fc_filing_date, doc_pending:doc_pending,fc_application_number:fc_application_number});
        });

        $(".fil_ded").change((function(){
        ...
        $(".final_deadline").change(function(){
            var final_deadline = $(this).val();
            var parent = $(this).parent().parent();
            var tmp = parent.attr('id');
            var file_id = tmp.replace(/doc_req_/,'');
            $.post("<?php echo base_url(); ?>fa/document_requirements/", {fa_id: fa_id, file_id: file_id, final_deadline: final_deadline});
        });
        ...
        $(".legalization").click(function(){
            ...
            var tmp = parent.attr('id');
            var file_id = tmp.replace(/doc_req_/,'');
            $.post("<?php echo base_url(); ?>fa/document_requirements/", {fa_id: fa_id, file_id: file_id, legalization: legalization, legalization_by: ''});
        });
        ...
        $(".tabs_container").tabs({
            ...
            select: function(event, ui) {
                ...
                if($(ui.tab).attr('href') == '#invoice') {

                    $('.stan_message_box_container .message_box').html('');
                    <?php if($associate_data->fa_invoice_status == 'approved') { ?>
                        var message = $('#popup_messages .message_box_success').clone();
                    <?php } elseif($associate_data->fa_invoice_status == 'pending-unlock' || $associate_data->fa_invoice_status == 'pending-approval') {?>

                    <?php $pending_popup = true; ?>
                        var message = $('#popup_messages .message_box_warning').clone();

                    <?php } elseif($associate_data->fa_invoice_status == 'rejected') { ?>
                        var message = $('#popup_messages .message_box_reject').clone();
                    <?php } elseif(!$associate_data->fa_invoice_status && $is_passed_country) { ?>
                    <?php
                    // this means filing deadline is passed and we need to show popup
                    $passed_deadline_popup = true; ?>

                        var message = $('#popup_messages .message_box_passed').clone();
                    <?php }  ?>
...
//                    var message = $('#popup_messages .message_box_info').clone();
                    ...
                    if (message) {
                        $('.stan_layout , .stan_message_box_container').show();
                    ...
                    if (error) {
                        ...
                        stanOverlay.setMESSAGE("Required file and have at least one additional fee.");
                        stanOverlay.SHOW();
                    }
        ...
        associate_data_id = '<?php echo $associate_data->id ?>';
        $('.message_box_passed').live('click' , function(){
            form_show();
            $('.stan_layout , .stan_message_box_container').hide();
        });

        $('.calculate_summ').live('keyup' , function(){
            calculate_total();
        });

        function calculate_total() {
            var summ = 0;
            $('.calculate_summ').each(function(key , value){
                if ($(value).val() != '') {
                    summ += parseFloat($(value).val());
                }
            });
            $('#total_summ').val(summ);
        }
...
//            console.log(value);
        ...
        $('#confirm_required').click(function () {
            ...
            $.post("<?php echo base_url(); ?>fa/document_required/", {country_id: id, case_id: case_id, value: '2', case_number: case_number});
        });

        $('.confirm_instructions').click(function () {
            var id = $('input[name = country_id_<?php echo $country_id; ?>]').val();
            var case_number = '<?= $case['case_number']?>';
            ...
            $.post("<?php echo base_url(); ?>fa/document_required/", {country_id: id, fa_id:fa_id, fa_note: fa_note, case_id: case_id, case_number: case_number, value: '1', files_id_arr: files_id_arr});
        });
...
<ul class="tabs">
    ...
    <li><a id="filing-conf_link" href="#filing-conf">Filing Confirmation</a></li>
...
<?php if ($case['case_type_id'] != '2') {?>
        <tr class="odd">
            ...
            <td><?php if(isset($case['first_priority_date']) && $case['first_priority_date']!='0000-00-00' && $case['first_priority_date'] !='00/00/00'){ echo date($this->config->item('client_date_format') , strtotime($case['first_priority_date']));}else{ echo "N/A";}?></td>
        </tr>
...
<div id="files">
    ...
    <table class="table files" id="switch_client_files">
        ...
        <?php if(!empty($case_files)): ?>
            <?php foreach ($case_files as $k => $file): ?>
                <?php
     ...
     foreach ($document_requirements_files as $k => $file){
        ...
        ?>
                <tr id="doc_req_<?php echo $file['file_id_link'] ?>" class="<?php echo $row_class ?>">
                            ...
                            <div class="notice">
                                <div>
                                    <input type="radio" class="legalization_choice" name="legalization_choice"
                                           id="legalization_apostille" value="apostille"/>&nbsp;
                    ...
                    <td class="extension">
                        <input type="text" class="datepicker final_deadline" name ='final_deadline' value="<?php if(isset($file['final_deadline'])) echo date($this->config->item('client_date_format') , strtotime($file['final_deadline'])); ?>">
                    </td>
                    <td class="late_fee">
...
<!--                            </div>-->
        ...
        <div class="additional_instructions">
            ...
            <textarea id="fa_note" placeholder="Your message goes here"></textarea>
...
<div id="filing-conf">
    ...
    <div>
        ...
        <div class="float-left filing-input">
            <input name="fc_filing_date" class="datepicker" value="<?php if(isset($filing_confirmation['filing_date'])) echo date($this->config->item('client_date_format') , strtotime($filing_confirmation['filing_date'])); ?>" type="text"/>
        </div>
        <div class="clear"></div>
    </div>
    <div>
        ...
        <div class="float-left filing-input">
            <div style="width: 200px">
                <?= form_dropdown('doc_pending', array('yes' => 'Yes', 'no' => 'No'), isset($filing_confirmation['doc_pending']) ? $filing_confirmation['doc_pending']: 'yes', 'id="d" class="doc_pending"');?>
                <div class="clear"></div>
            </div>
        </div>
        <div class="clear"></div>
    ...
    <div>
        ...
        <div class="float-left filing-input">
            <input name="examenation" class="datepicker" value="<?php if(isset($filing_confirmation['examenation'])) echo date($this->config->item('client_date_format') , strtotime($filing_confirmation['examenation'])); ?>" type="text"/>
    ...
    <div>
        ...
        <div class="float-left filing-input">
            <input name="fc_application_number" value="<?php if(isset($filing_confirmation['application_number'])) echo $filing_confirmation['application_number'];?>" type="text"/>
    ...
    <p>Please upload your filing report for <b><?php echo $current_country['country'] ?> ONLY</b> here:</p>
...
<style type="text/css">
    ...
    <div id="popup_messages">
        ...
        <div class="message_box message_box_warning">
            ...
            <div class="info_close"></div>
            <div class="info_center">
                <div class="info_Information">Your
                    <?php if($associate_data->fa_invoice_status == 'pending-approval') { ?>
                        invoice
                    <?php } else { ?>
                        request
                    <?php } ?>
                    is being reviewed</div>
                <div class="info_message"></div>
            </div>
        </div>
        <div class="clear"></div>
        <div class="message_box message_box_error message_box_reject">
            <div class="info_part"></div>
            <div class="info_close">&nbsp;</div>
            <div class="info_center">
        ...
        <div class="message_box message_box_info">
            <div class="info_part"></div>
            <div class="info_close"></div>
            <div class="info_center">
                <div class="info_Information">You have to upload invoice</div>
                <div class="info_message"></div>
            </div>
        </div>
        <div class="clear"></div>
    ...
    <?php
    if($associate_data->fa_invoice_status == 'approved' || !empty($passed_deadline_popup) || !empty($pending_popup)) {
        $disabled = 'disabled="true"';
    } else {
        $disabled = false;
    }

    if($associate_data->fee_currency == 'usd') {
        $currency_sign = '$';
    } else {
        $currency_sign = '&euro;';
    }
    ...
    <div class="country_invoice country_invoice_<?php echo $current_country['id'] ?>">
        ...
        <form class="invoice_form" action="<?php echo base_url() ?>fa/case_fees/<?php echo $this->uri->segment(4) ?>" method="post" enctype="multipart/form-data">
            ...
            <div>
                ...
                <div class="float-left invoice-input">
                    <span class="nobr"><?php echo $currency_sign ?><input class="calculate_summ" readonly="true" type="text" name="fa_invoice_professional_fee" value="<?php echo $associate_data->fee ?>"/></span>
                </div>
                <div class="clear"></div>
            </div>
            <div>
                ...
                <div class="float-left invoice-input">
                    <span class="nobr"><?php echo $currency_sign ?><input class="calculate_summ" <?php echo $disabled ?> name="fa_invoice_official_fee" type="text" value="<?php echo $associate_data->fa_invoice_official_fee ?>" /></span>
                </div>
                <div class="clear"></div>
            </div>
            <div id="additional_fees">
                <div class="additional_fee_block">
                    ...
                    <div class="float-left invoice-input">
                        <span class="nobr"><?php echo $currency_sign ?><input class="calculate_summ" <?php echo $disabled ?> name="additional_fee_by_fa[]" type="text"/ ></span>
                    </div>
                ...
                <?php foreach($invoice_additional_fees as $key => $additional_fee) { ?>
                    <div>
                        ...
                        <div class="float-left invoice-input">
                            <span class="nobr"><?php echo $currency_sign ?><input class="calculate_summ" <?php echo $disabled ?> name="additional_fee_by_fa_update[]" type="text" value="<?php echo $additional_fee->additional_fee_by_fa ?>" /></span>
                        </div>
                        <div class="float-left invoice-additional">
                ...
                <div>
                    ...
                    <div class="float-left invoice-input">
                        <span class="nobr"><?php echo $currency_sign ?><input <?php echo $disabled ?> class="calculate_summ" name="additional_fee_by_fa[]" type="text"/ ></span>
                    </div>
                    <div class="float-left invoice-additional">
                        <input <?php echo $disabled ?> name="additional_fee_description_by_fa[]" type="text" placeholder="Please provide a short description of this cost"/>
                    </div>
        ...
        <div>
            ...
            <div class="float-left invoice-input">
                <span class="nobr"><?php echo $currency_sign ?><input disabled="true" id="total_summ" type="text"/></span>
            </div>

            <div class="clear"></div>
        </div>
        <?php if(!$disabled) { ?>
            <p>Please upload your invoice for <?php echo $current_country['country'] ?> ONLY</p>
                <!-- UPLOADER STARTS -->
                ...
                <script type="text/javascript">
                    ...
                    <?php if($_SERVER['HTTP_HOST'] == 'zen' || $_SERVER['HTTP_HOST'] == 'lastzenfile') {
                    ...
                    $(function() {
                        $('#file_upload').uploadify({
                            ...
                            'uploader': base_url + 'upload/do_upload/<?php echo $case['case_number'] ?>',
                            'formData'      : {'case_associate_data_id' : '<?php echo $associate_data->id ?>' , 'country_id' : '<?php echo $associate_data->country_id ?>'},
                            'fileTypeExts':'*.jpg;*.bmp;*.png;*.tif;*.txt;*.pdf',
        ...
        <?php } ?>
            <div id="invoices">
        <?php if($fa_invoices) { ?>
                <?php $object->get_invoices_table($associate_data->id , $disabled); ?>
        <?php } ?>
            </div>
        <?php if(!$disabled) { ?>
