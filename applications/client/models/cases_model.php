...
//        }
        ...
        if (!empty($CASE)) {
            ...
            if (!empty($TEMPLATE)) {
                ...
                else
                {
                    if ($tpl_num == '36') {
                        $to = $BDV_EMAIL;
                        $this->send_emails->send_email($from, $from, $TEMPLATE["subject"], $TEMPLATE["content"], $to, $cc);
                        return true;
                    }
                    if ($CASE['manager_id']) {
                    ...
                    if ($managers) {
                        foreach ($managers as $manager)
                        {
                            ...
                            $to = $manager["email"];
                            $this->send_emails->send_email($from, $from, $temp["subject"], $temp["content"], $to, $cc);
    ...
    public function get_filing_confirmation($case_id, $country_id, $fa_id)
    {

        $this->db->select('tracker_additional_data.*, cases_tracker.fr_filing_date');
    ...
    public function get_fa_case_files($case_id, $country_id, $types_array = '', $visible_to_fa = true)
    {
    ...
    public function update_tracker_additional_data_id($data, $case_id, $country_id)
    {
        $this->db->where('case_id', $case_id);
        $this->db->where('country_id', $country_id);
        $query = $this->db->get('cases_tracker');
        $tmp = $query->row_array();
        if (!empty($tmp['tracker_additional_data_id'])) {
            $this->db->where('id', $tmp['tracker_additional_data_id']);
            $this->db->update('tracker_additional_data', $data);
        }
        else {
            $this->db->insert('tracker_additional_data', $data);
            $tracker_id = $this->db->insert_id();
            $this->db->set('tracker_additional_data_id', $tracker_id);
            $this->db->where('case_id', $case_id);
            $this->db->where('country_id', $country_id);
    ...
    public function update_tracker_filing_date($filing_date, $case_id, $country_id)
    {

        $this->db->set('fr_filing_date', $filing_date);
        $this->db->where('case_id', $case_id);
        $this->db->where('country_id', $country_id);
        $this->db->update('cases_tracker');
    }
    ...
    public function cases_associates_table($case_id, $country_id)
    {
        $this->db->where('case_id', $case_id);
        $this->db->where('country_id', $country_id);
        $this->db->where('is_active', '1');
        $query = $this->db->get('cases_associates_data');
        return $tmp = $query->row_array();
    }
    ...
    public function fa_active_cases($fa_id, $params = '')
    {
        ...
        $this->db->select('cases.*,countries.country,countries.country_filing_deadline, cases_associates_data.reference_number as fa_reference_number');
    ...
    {
        $this->db->select('cases.*,countries.country,cases_associates_data.reference_number as fa_reference_number');
        $this->db->join('cases', 'cases_associates_data.case_id = cases.id');
        $this->db->join('countries', 'cases_associates_data.country_id = countries.id');
        $this->db->join('cases_tracker', 'zen_cases_tracker.country_id = zen_cases_associates_data.country_id AND zen_cases_associates_data.case_id = zen_cases_tracker.case_id');
        $this->db->where('cases_associates_data.associate_id', $fa_id);
    ...
    public function document_required_tracker($value, $case_id, $country_id)
    {
    ...
    public function set_fa_files_visible_to_pm($files_id, $fa_id)
    {
    ...
    public function insert_fa_case_note($fa_id, $fa_note, $case_number)
    {
        $note = array(
            'note' => $fa_note,
            'case_number' => $case_number,
